...
int main(int argc, char *argv[]) {
    ...
    if(connection == LOCAL){
        if((sockfd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) { // socket()
        ...
        if((connect(sockfd, (struct sockaddr *)&addr, sizeof(struct sockaddr))) == -1){ //connect()
    ...
    if((write(sockfd, myname, sizeof(myname))) == -1){
            ...
            if(beginner){
                while(1){
                    ...
                    if(fork() == 0){
                        ...
                        if((write(sockfd, move, sizeof(move))) == -1){
                            perror("Error while writing to socket");
                        }
                        printf("Sent: %s\n", move);
                        close(sockfd);
