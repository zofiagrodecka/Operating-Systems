...
int main(int argc, char *argv[]) {
    ...
    void *handle = dlopen("libmergelib.so", RTLD_LAZY);
    if(!handle){
        perror("Library not imported.");
        exit(-4);
    }

    void (*lib_create_main_array)(struct main_array *, int);
    lib_create_main_array = (void (*)(struct main_array *, int))dlsym(handle,"create_main_array");
    if(dlerror() != NULL){
        perror("Function create_main_array() not loaded.");
        exit(-4);
    }

    struct files_couple * (*lib_create_couple)(char *);
    lib_create_couple = ( struct files_couple * (*)(char *))dlsym(handle,"create_couple");
    if(dlerror() != NULL){
        perror("Function create_couple() not loaded.");
        exit(-4);
    }

    int (*lib_merge_files)(char *, char *, char *);
    lib_merge_files = (int (*)(char *, char *, char *))dlsym(handle,"merge_files");
    if(dlerror() != NULL){
        perror("Function merge_files() not loaded.");
        exit(-4);
    }

    int (*lib_load_file)(char *, struct files_couple *, int, struct main_array *);
    lib_load_file = (int (*)(char *, struct files_couple *, int, struct main_array *))dlsym(handle,"load_file");
    if(dlerror() != NULL){
        perror("Function load_file() not loaded.");
        exit(-4);
    }

    void (*lib_remove_row)(int, int, struct main_array *);
    lib_remove_row = (void (*)(int, int, struct main_array *))dlsym(handle,"remove_row");
    if(dlerror() != NULL){
        perror("Function remove_row() not loaded.");
        exit(-4);
    }

    void (*lib_remove_block)(int, struct main_array *);
    lib_remove_block = (void (*)(int, struct main_array *))dlsym(handle,"remove_block");
    if(dlerror() != NULL){
        perror("Function remove_block() not loaded.");
        exit(-4);
    }
        ...
        if(strcmp(argv[i], "create_table") == 0){
            ...
            switch(oper){
                ...
                case mergeFiles:
                    ...
                    filesCouple = lib_create_couple(files);
                    sprintf(fileindex, "%d", i);
                    strcpy(outputfilename, "merged");
                    strcat(strcat(outputfilename, fileindex), txt); // outputfilename
                    filesCouple->rows_num = lib_merge_files(filesCouple->filename1, filesCouple->filename2, outputfilename);
                    lib_load_file(outputfilename, filesCouple, filesCouple->rows_num, &array);
                ...
                case removeRow:
                    ...
                    if(counter == 0){
                        ...
                        lib_remove_row(blockInd, n, &array);
